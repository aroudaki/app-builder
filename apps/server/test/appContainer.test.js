...
import { appContainerTool } from '../dist/src/langgraph/tools/definitions.js';
...
describe('App Container Tool', () => {
    ...
    describe('Direct AppContainer Class', () => {
    ...
    describe('LangGraph Tool Integration', () => {
        test('should have valid tool schema', () => {
            expect(appContainerTool.name).toBe('app_container');
            expect(appContainerTool.description).toContain('Docker container environment');
            expect(appContainerTool.schema).toBeDefined();
        });
        ...
        test('should validate tool input with Zod schema', () => {
            ...
            // Test invalid input
            expect(() => {
                appContainerTool.schema.parse({ invalid: 'input' });
            }).toThrow();
        });

        test('should execute tool and return JSON result', async () => {
        ...
        test('should handle tool execution errors gracefully', async () => {
            const result = await appContainerTool.invoke({ command: 'nonexistent-command' });

            const parsed = JSON.parse(result);
            expect(parsed.success).toBe(false);
            expect(parsed).toHaveProperty('error');
            expect(parsed.exitCode).toBe(127); // Command not found
        });

        test('should return formatted output for successful commands', async () => {
            const result = await appContainerTool.invoke({ command: 'pwd' });

            const parsed = JSON.parse(result);
            expect(parsed.success).toBe(true);
            expect(parsed.output).toContain('/app');
